<script setup>
  ...
  let dd = function (date) {return String(date.getDate()).padStart(2, '0')}
  let mm = function (date) {return String(date.getMonth() + 1).padStart(2, '0')}
  let yyyy = function (date) {return date.getFullYear()}
  let hh = function (date) {return String(date.getHours()).padStart(2, '0')}
  let min = function (date) {return String(date.getMinutes()).padStart(2, '0')}

  if (yyyy(now) !== yyyy(date)) {
    messageDate += dd(date) + "/" + mm(date) + "/" + yyyy(date) + " "
  } else if (mm(now) !== mm(date) || dd(now) !== dd(date)) {
    messageDate += dd(date) + "/" + mm(date) + " "
  }

  messageDate += hh(date) + ":" + min(date)
</script>
