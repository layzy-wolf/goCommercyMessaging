...
func (c *chatClient) ForwardMessage(ctx context.Context, opts ...grpc.CallOption) (Chat_ForwardMessageClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[0], Chat_ForwardMessage_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &chatForwardMessageClient{stream}
	return x, nil
}

type Chat_ForwardMessageClient interface {
	Send(*Message) error
	Recv() (*Message, error)
	grpc.ClientStream
}

type chatForwardMessageClient struct {
	grpc.ClientStream
}

func (x *chatForwardMessageClient) Send(m *Message) error {
	return x.ClientStream.SendMsg(m)
}

func (x *chatForwardMessageClient) Recv() (*Message, error) {
	m := new(Message)
...
func (c *chatClient) GetMessages(ctx context.Context, in *GetRequest, opts ...grpc.CallOption) (*ChatMessages, error) {
...
func (UnimplementedChatServer) ForwardMessage(Chat_ForwardMessageServer) error {
	return status.Errorf(codes.Unimplemented, "method ForwardMessage not implemented")
}
...
func _Chat_ForwardMessage_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChatServer).ForwardMessage(&chatForwardMessageServer{stream})
}

type Chat_ForwardMessageServer interface {
	Send(*Message) error
	Recv() (*Message, error)
	grpc.ServerStream
}

type chatForwardMessageServer struct {
	grpc.ServerStream
}

func (x *chatForwardMessageServer) Send(m *Message) error {
	return x.ServerStream.SendMsg(m)
}

func (x *chatForwardMessageServer) Recv() (*Message, error) {
	m := new(Message)
...
func _Chat_GetMessages_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ChatServer).GetMessages(ctx, req.(*GetRequest))
	}
...
var Chat_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ForwardMessage",
			Handler:       _Chat_ForwardMessage_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
